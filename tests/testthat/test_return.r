...
test_that("Column names of data.table is correct", {
    expect_equal(colnames(run_adPW_short(dat)),
                 c('term.id', 'term.name', 'p.val', 'term.size', 'overlap'))
    expect_equal(colnames(run_adPW_short_contribution(dat)),
...
test_that("All results or only significant ones are returned", {
    expect_true(all(activeDriverPW(dat, gmt, cytoscape.filenames=NULL, contribution=FALSE)$p.val < 0.05))
    expect_equal(nrow(activeDriverPW(dat, gmt, cytoscape.filenames=NULL,
                                     contribution=FALSE, return.all=TRUE)), length(gmt))
})


test_that("No significant results are found", {
    expect_warning(res1 <- activeDriverPW(dat, gmt, cytoscape.filenames=NULL,
    ...
    expect_warning(res2 <- activeDriverPW(dat, gmt, cytoscape.filenames=NULL,
    ...
    expect_warning(res3 <- activeDriverPW(dat, gmt, cytoscape.filenames=NULL,
