...
Now we merge the P-values of the two datasets using directional assumtions and compare these with the plain non-directional merging. The top 5 scoring genes differ if we penalise genes where this directional logic is violated: While 4 of 5 genes retain significance, the gene PIK3R4 is penalised. Interestingly, as a consequence of penalizing PIK3R4, other genes such as ITGB2 move up in rank.
...
First perform integrative pathway enrichment analysis with no directionality. Then perform directional integration and pathway enrichment analysis. For this analysis the directional coefficients and expected_direction are from the gene-based analysis described above.
...
This enrichment map requires some manual aggregation. The aggregated subgroups.txt file keeps track of lost/gained/maintained pathways between the two methods.
...
all_pathways$evidence <- append(enriched_pathways$evidence, enriched_pathways_directional$evidence)
...
df_pathways <- data.frame(term_id = c(enriched_pathways$term_id,enriched_pathways_directional$term_id),
