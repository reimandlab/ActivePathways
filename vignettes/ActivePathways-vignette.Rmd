...
## Merging p-values
A key feature of `ActivePathways` is the integration of multiple complementary omics datasets to prioritise genes for the pathway analysis. In this approach, genes with significant scores in multiple datasets will get the highest priority, and certain genes with weak scores in multiple datasets may be ranked higher, highlighting functions that would be missed when only single datasets were analysed. `ActivePathways` accomplishes this by merging the series of p-values in the columns of the scores matrix for each gene into a single combined P-value. The four methods to merge P-values are Fisher's method (the default), Brown's method (extension of Fisher's), Stouffer's method and Strube's method (extension of Stouffer's). The Brownâs and Strube's methods are more conservative in the case when the input datasets show some large-scale similarities (i.e., covariation), since they will take that into account when prioritising genes across similar datasets. The Brownâs or Strube's method is recommended for most cases since omics datasets are often not statistically independent of each other and genes with high scores in one dataset are more likely to have high scores in another dataset just by chance. 

The following example compares the merged P-values of the first few genes between the four methods. Fisher's and Stouffer's method are two alternative strategies to merge p-values and as a result the top scoring genes and p-values may differ. The genes with the top scores for Brown's method are the same as Fisher's, but their P-values are somewhat more conservative. This is the case for Strube's method as well, in which the top scoring genes are the same as Stouffer's method, but the P-values are more conservative. Fisher's method is more sensitive to a single small p-value than Stouffer's method, which is why the merged P-values are much more conservative with Stouffer's method in this example. 
```{r}
...
The scores_direction and expected_direction parameters have recently been introduced to merge_p_values() and ActivePathways() functions, which can incorporate directional logic into the p-value merging methods. For example, fold-change in protein expression would be expected to associate positively with mRNA change of the corresponding gene, while DNA methylation change of the gene promoter would be expected to associate negatively. We penalize genes where such directional interaction assumptions are violated. Using the expected_direction parameter we can encode our expected relationship between different datasets, and scores_direction would reflect the log2 fold-change values of each gene. The magnitude of the input log2 fold-change values in scores_direction is discarded and only the directional information is used for pathway enrichment analysis and merging p-values. 

As an example, the human glioma cell transcriptome was profiled from wild-type, overexpression and knockdown long noncoding RNA (lncRNA) HOXA10-AS conditions. The differentially expressed genes in HOXA10-AS overexpression and knockdown experiments in brain cancer cells provided two datasets that are expected to have opposite fold-change directionality.
```{r}
df_oe_kd <- read.table(system.file('extdata', 'Differential_expression_hoxa10as.tsv', package = 'ActivePathways'), 
header = TRUE, sep = '\t')
df_oe_kd
```


The top 5 scoring genes differ if we penalize genes where this directional logic is violated. Using Stouffer's method the genes, COL25A1, ARHGEF17,B4GALT6 and NEGR1 are penalized, whilst CPED1 retains its significance and improves in rank. 

```{r}
scores3 <- data.frame(row.names = df_oe_kd[,1], overexpression = df_oe_kd[,3], knockdown = df_oe_kd[,5])
scores3 <- as.matrix(scores3)
scores3[is.na(scores3)] <- 1

scores_direction <- data.frame(row.names = df_oe_kd[,1], overexpression = df_oe_kd[,2], knockdown = df_oe_kd[,4])
scores_direction <- as.matrix(scores_direction)
scores_direction[is.na(scores_direction)] <- 1

expected_direction <- c(-1,1)

sort(merge_p_values(scores3, 'Stouffer'))[1:5]
sort(merge_p_values(scores3, 'Stouffer',scores_direction, expected_direction))[1:5]
df_oe_kd[df_oe_kd$gene_name %in% c('COL25A1','CPED1','ARHGEF17','B4GALT6','NEGR1'),]
```
...
```{r}
res[res$term_id == "REAC:422475","evidence"]
```
