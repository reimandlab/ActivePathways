...
From a matrix of p-values, `activeDriverPW` creates a gene list query by merging the values to determine a single score for each gene, then filtering and sorting this list of p-values. This creates a list of genes - which we'll creatively call the `genelist` - ordered most significant first. `activeDriverPW` tests if a pathway is 'enriched', or over-represented in our `genelist`, by performing a hypergeometric test (aka a Fisher's exact test) on the `genelist` and the genes annotated to the pathway. But rather than testing the entire `genelist`, `activeDriverPW` performs incremental enrichment analysis with an increasingly larger subset of `genelist` starting at the top of the list, and takes the subset of `genelist` that yields the smallest p-value. This approach is useful when scores are of varying biological importance as it identifies the genes most relevant to the enrichment of the pathway.
...
```
This filtering can also be done automatically using the `geneset.filter` option to the `activeDriverPW` function. By default this filter removes any gene sets with less than 5 or more than 1000 annotated genes. Our stricter filter can be applied inside `activeDriverPW` using the option `geneset.filter=c(10, 500)`
...
## Merging p-values
Our input file consisted of multiple scores for each gene, but in order to perform pathway enrichment analysis each gene needs to have a single score. `activeDriverPW` accomplishes this by merging the p-values for each gene into a single combined score. The two main methods to merge p-values are Fisher's method (the default) and Brown's method, but other methods are also available. You can preview the merged p-values using the hidden merge_p_values method
```{r}
scores
activeDriverPW:::merge_p_values(scores, 'Fisher')
activeDriverPW:::merge_p_values(scores, 'Brown')
```
...
```{r}
scores2 <- cbind(scores[, 'CDS'], activeDriverPW:::merge_p_values(scores[, c('X3UTR', 'X5UTR', 'promCore')], 'Fisher'))
...
## Cutoff
In addition to removing any genes not found in the background, `activeDriverPW` also removes any genes that have a very high p-value after merging the scores. This threshold represents the maximum p-value for a gene to be considered of interest in our analysis. The threshold is `0.1` by default, but can be changed using the `cutoff` option.
